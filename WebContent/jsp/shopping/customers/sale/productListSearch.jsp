...
<body>
	 ...
	 <table id="borderblack" width="300px">	    	
	    	...
	    	<td id="borderblack" width="30%"><img src="<s:url value='/images/%{#user.type_id}/%{#user.code}.jpg' />" width="80px" height="120px" /></td>
		    	...
		    	<td align="center" id="borderblack" width="70%">
			    	<s:text name="รหัส"/>&nbsp;&nbsp;<s:property value="#user.code" /><br>
			    	<s:property value="#user.product_desc" /><br>
			    	<s:text name="ราคา"/>&nbsp;&nbsp;<s:property  value="#user.price" />&nbsp;&nbsp;<s:text name="บาท"/><br>
			    	<s:text name="จำนวน"/>&nbsp;&nbsp;<s:property value="#user.stock_num" />&nbsp;&nbsp;<s:text name="ชิ้น"/><br><br>
			    		<s:if test="#user.stock_num != 0" >
			    			<s:submit name="%{#user.code}" value="Add to Cart" onclick="addchart(name)"> </s:submit>    			
			    		</s:if>
			    		<s:else>
			    			<h4><s:text name="** ขออภัยสินค้าหมด **" /></h4>
			    		</s:else>
   ...
   <thead>
    <tr>
     <th class="left"><s:text name="จำนวน" />&nbsp;<s:property value="criteria.totalResult" />&nbsp;<s:text name="ชิ้น"/></th>
     <th class="right">
